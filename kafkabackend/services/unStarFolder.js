...
function handle_request(msg, callback){
    ...
    mongo.getConnection((connectionNumber,db)=>{
        ...
        foldersCollection.findOneAndUpdate({"folderid":folderid},{$set:{"starred":false}}, function(err, folderData){
            if (err) {
                console.error(err);
                res.code="202";
                res.data="folder unstarred failed";
                callback(null,res);
            }
            else{
                res.code="201";
                res.data="folder unstarred";
                callback(null,res);  
            }
